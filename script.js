...
function setupEventListeners() {
    ...
    if (removeAllPlayersBtn) removeAllPlayersBtn.addEventListener('click', removeAllPlayers);
...
function updateAdminPlayersList() {
    ...
    if (players.length === 0) {
        adminPlayersList.innerHTML = '<div class="no-games-message">Nessun partecipante iscritto.</div>';
        return;
    }
    ...
    sortedPlayers.forEach(player => {
        ...
        playerItem.className = 'admin-game-item';

        const playerInfo = document.createElement('div');
        playerInfo.className = 'game-info';
        playerInfo.appendChild(createAvatarWithInitials(player.name, player.photoURL));

        const playerDetails = document.createElement('div');
        const playerName = document.createElement('strong');
        playerName.textContent = player.name || '(senza nome)';
        const playerEmail = document.createElement('div');
        playerEmail.style.fontSize = '0.9rem';
        playerEmail.style.color = 'var(--text-secondary)';
        ...
        playerActions.className = 'game-actions';

        const removeBtn = document.createElement('button');
        removeBtn.className = 'btn-danger';
        removeBtn.innerHTML = '<i class="fas fa-user-minus"></i> Rimuovi';
        removeBtn.addEventListener('click', () => removePlayer(player.id, player.name || player.email || 'questo partecipante'));
...
// Player Management
// Rimuove il giocatore dal tabellone (nodo 'players') e i suoi punteggi.
// L'account su Firebase Authentication resta intatto: ensurePlayerExists()
// lo reinserisce al prossimo login, senza punti.
async function removePlayer(playerId, playerName) {
    if (!confirm(`Rimuovere ${playerName} dal tabellone? Verranno cancellati anche i suoi punteggi.\n\nL'account di accesso NON viene eliminato: al prossimo login tornerà nell'elenco senza punti.`)) {
        return;
    }
...
async function removeAllPlayers() {
    if (players.length === 0) {
        alert('Non ci sono partecipanti da rimuovere');
        return;
    }

    if (!confirm(`Rimuovere tutti i ${players.length} partecipanti dal tabellone? Verranno cancellati anche i loro punteggi.\n\nGli account di accesso NON vengono eliminati: ognuno tornerà nell'elenco al suo prossimo login, senza punti.`)) {
        return;
    }
    ...
    try {
        ...
        console.error('Errore rimozione partecipanti:', error);
        alert('Errore durante la rimozione dei partecipanti');
        hideLoading();
    }
}
...
function getSportIcon(sport) {
    const icons = {
        ...
        'tennis': '<i class="fas fa-baseball"></i>',
        'videogiochi': '<i class="fas fa-gamepad"></i>',
        'azzardo': '<i class="fas fa-dice"></i>',
