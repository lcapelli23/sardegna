...
function showMainScreen() {
    ...
    // Usa le iniziali invece del placeholder che non funziona
    if (currentUser.photoURL) {
        userPhoto.src = currentUser.photoURL;
    } else {
        const initials = getInitials(currentUser.displayName || currentUser.email);
        userPhoto.src = `data:image/svg+xml,${encodeURIComponent(`
...
async function loadData() {
    ...
    try {
        ...
        // Converte gli oggetti punteggio in numeri semplici
        scores = {};
        Object.keys(scoresData).forEach(playerId => {
            scores[playerId] = {};
            Object.keys(scoresData[playerId] || {}).forEach(gameId => {
                const scoreData = scoresData[playerId][gameId];
                // Se è un oggetto con campo points, estrai il valore, altrimenti usa il valore diretto
                scores[playerId][gameId] = typeof scoreData === 'object' && scoreData.points !== undefined 
                    ? scoreData.points 
                    : scoreData;
            });
        });
...
function loadDemoData() {
    ...
    scores = {
        'player1': { 'game1': 85, 'game2': 70, 'game3': 90 }, // Totale: 245
        'player2': { 'game1': 85, 'game2': 70, 'game3': 90 }, // Totale: 245 (pari merito con player1)
        'player3': { 'game1': 80, 'game2': 80, 'game3': 80 }  // Totale: 240
    };
...
function updateLeaderboard() {
    ...
    playerTotals.forEach((player) => {
        const row = createPlayerRow(player, player.position);
...
function createPlayerRow(player, position) {
    ...
    // Usa le iniziali se non c'è photoURL
    if (player.photoURL) {
        ...
        avatar.src = `data:image/svg+xml,${encodeURIComponent(`
            <svg width="35" height="35" xmlns="http://www.w3.org/2000/svg">
                <circle cx="17.5" cy="17.5" r="17.5" fill="#1e40af"/>
                <text x="17.5" y="22" text-anchor="middle" fill="white" font-family="Arial" font-size="12" font-weight="bold">${initials}</text>
            </svg>
        `)}`;
    }
...
function updatePointsManagement() {
    ...
    players.forEach(player => {
        ...
        // Crea avatar con iniziali se non c'è photoURL
        let avatarSrc;
        if (player.photoURL) {
            avatarSrc = player.photoURL;
        } else {
            const initials = getInitials(player.name);
            avatarSrc = `data:image/svg+xml,${encodeURIComponent(`
                <svg width="35" height="35" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="17.5" cy="17.5" r="17.5" fill="#1e40af"/>
                    <text x="17.5" y="22" text-anchor="middle" fill="white" font-family="Arial" font-size="12" font-weight="bold">${initials}</text>
                </svg>
...
async function savePointsEdit() {
    ...
    try {
        ...
        // Aggiorna dati locali con il valore numerico semplice
        if (!scores[editingPlayerId]) {
            scores[editingPlayerId] = {};
        }
        scores[editingPlayerId][editingGameId] = newPoints;
