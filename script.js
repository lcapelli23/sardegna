...
function createGameLeaderboard(game) {
    ...
    title.textContent = `${getSportName(game.sport)} - ${game.name}`;
    ...
    tableHeader.style.gridTemplateColumns = '60px minmax(200px, 1fr) 100px 100px';
    ...
    totalHeader.textContent = 'Totale Generale';
    ...
    const playerGameScores = players.map(player => {
        ...
        // Totale generale di tutte le gare
        const totalScore = games.reduce((sum, g) => {
            return sum + (scoreObj[g.id]?.points || 0);
        }, 0);
...
function createGamePlayerRow(player, position, game) {
    const row = document.createElement('div');
    row.className = 'player-row';
    row.style.gridTemplateColumns = '60px minmax(200px, 1fr) 100px 100px';
