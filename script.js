...
const firebaseConfig = {
    // NOTA: Sostituire con la propria configurazione Firebase
    apiKey: "your-api-key",
...
function showMainScreen() {
    ...
    // Per utenti registrati con email, usa un avatar di default
    userPhoto.src = currentUser.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.displayName || currentUser.email)}&background=1e40af&color=fff&size=40`;
}
...
async function loadData() {
    try {
        ...
        // Carica giochi
        const gamesSnapshot = await db.collection('games').orderBy('createdAt').get();
        ...
        scoresSnapshot.docs.forEach(doc => {
            ...
            scores[data.playerId][data.gameId] = data.points;
        });
...
function createPlayerRow(player, position) {
    ...
    avatar.src = player.photoURL || 'https://via.placeholder.com/35';
...
async function deleteGame(gameId) {
    ...
    try {
        ...
        const scoresSnapshot = await db.collection('scores').where('gameId', '==', gameId).get();
        ...
        Object.keys(scores).forEach(playerId => {
            if (scores[playerId][gameId] !== undefined) {
...
async function savePointsEdit() {
    ...
    try {
        ...
        // Cerca se esiste già un record per questo giocatore e gara
        const existingScoreQuery = await db.collection('scores')
            .where('playerId', '==', editingPlayerId)
            .where('gameId', '==', editingGameId)
            .get();
        ...
        if (existingScoreQuery.empty) {
            ...
            const docId = existingScoreQuery.docs[0].id;
...
function getSportIcon(sport) {
    const icons = {
        ...
        'tiro-arco': '<i class="fas fa-bow-arrow"></i>'
    };
